...
function renderArenaSelection() {
  ...
  wrap.innerHTML = owned.map(c => `<div class="arena-team-pick ${active && active.id===c.id ? 'active':''}" onclick="setActiveCompanion('${c.id}');renderArenaSelection()"><div style="height:80px;background-size:800px 500px;background-position:${getPortraitPositionByCompanionId(c.id)}"></div><div style="margin-top:6px;font-family:var(--font-tact);font-size:10px">${c.name}</div></div>`).join('');
}
