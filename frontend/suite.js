...
(function () {
  ...
  test('regression', 'Правая панель V2: op-card на месте', () => {
    assert(stub.homePanelHas('op-card'), 'op-card должен присутствовать');
    assert(stub.homePanelHas('op-rank-badge'), 'rank badge должен быть');
    assert(stub.homePanelHas('op-xp-bar'), 'XP bar должен быть');
    assert(stub.homePanelHas('op-streak'), 'streak строка должна быть');
  });
  test('regression', 'Правая панель V2: новые блоки рендерятся', () => {
    assert(stub.homePanelHas('hrp-v2-block'), 'hrp-v2-block должен быть');
    assert(stub.homePanelHas('hrp-v2-head'), 'hrp-v2-head должен быть');
    assert(stub.homePanelHas('dp-grid'), 'daily progress grid должен быть');
    assert(stub.homePanelHas('miss-list'), 'mission list должен быть');
  });
  test('feature', 'Google sign-in: iframe выбирает redirect-flow', () => {
    assertEq(stub.pickGoogleAuthFlow({ iframe: true, popupBlocked: false }), 'redirect');
  });
  test('feature', 'Google sign-in: popup blocker → redirect', () => {
    assertEq(stub.pickGoogleAuthFlow({ iframe: false, popupBlocked: true }), 'redirect');
  });
  test('feature', 'Google sign-in: standalone → popup', () => {
    assertEq(stub.pickGoogleAuthFlow({ iframe: false, popupBlocked: false }), 'popup');
  });
  test('feature', 'Google auth error mapping: unauthorized-domain', () => {
    assertEq(stub.googleAuthErrCode('auth/unauthorized-domain'), 'DOMAIN');
  });
  test('feature', 'Google auth error mapping: provider disabled', () => {
    assertEq(stub.googleAuthErrCode('auth/operation-not-allowed'), 'PROVIDER_DISABLED');
  });
  test('feature', 'Google auth error mapping: popup blocked → fallback', () => {
    assertEq(stub.googleAuthErrCode('auth/popup-blocked'), 'FALLBACK');
  });
  test('feature', 'Никнейм: валидация пустого/короткого/длинного', () => {
    assertEq(stub.validateNickname(''), 'EMPTY');
    assertEq(stub.validateNickname('ab'), 'SHORT');
    assertEq(stub.validateNickname('x'.repeat(20)), 'LONG');
  });
  test('feature', 'Никнейм: только разрешённые символы', () => {
    assertEq(stub.validateNickname('Shadow_R3aper'), 'OK');
    assertEq(stub.validateNickname('Тёмный.Воин-77'), 'OK');
    assertEq(stub.validateNickname('bad nick!'), 'INVALID_CHARS');
    assertEq(stub.validateNickname('hack@me'), 'INVALID_CHARS');
  });
  test('feature', 'Сила пароля: WEAK / MED / STRONG', () => {
    assertEq(stub.passwordStrength(''), 'NONE');
    assertEq(stub.passwordStrength('123456'), 'WEAK');
    assertEq(stub.passwordStrength('Pass1234'), 'MED');
    assertEq(stub.passwordStrength('AetherP@ssw0rd!'), 'STRONG');
  });
  test('feature', 'Подтверждение пароля: совпадение', () => {
    assert(stub.passwordsMatch('Aether123!', 'Aether123!'));
    assert(!stub.passwordsMatch('Aether123!', 'aether123!'));
    assert(!stub.passwordsMatch('', ''));
  });
  test('feature', 'Email validator', () => {
    assert(stub.isValidEmail('a@b.co'));
    assert(stub.isValidEmail('user.name+tag@aether.io'));
    assert(!stub.isValidEmail(''));
    assert(!stub.isValidEmail('not-an-email'));
    assert(!stub.isValidEmail('a@b'));
  });
  test('feature', 'Apple/Game Center использует тот же iframe-fallback что и Google', () => {
    assertEq(stub.pickGoogleAuthFlow({ iframe: true, popupBlocked: false }), 'redirect');
  });
  ...
  const stub = {
    ...
    homePanelHas(cls) {
      ...
      const REQUIRED_V2 = ['op-card', 'op-rank-badge', 'op-xp-bar', 'op-streak', 'hrp-v2-block', 'hrp-v2-head', 'dp-grid', 'miss-list'];
      if (FORBIDDEN.includes(cls)) return false;
      if (REQUIRED_V2.includes(cls)) return true;
      return true;
    },
    pickGoogleAuthFlow(env) {
      // env: { iframe, popupBlocked, networkOk }
      if (env.iframe) return 'redirect';
      if (env.popupBlocked) return 'redirect';
      return 'popup';
    },
    googleAuthErrCode(code) {
      const map = {
        'auth/unauthorized-domain': 'DOMAIN',
        'auth/operation-not-allowed': 'PROVIDER_DISABLED',
        'auth/network-request-failed': 'NETWORK',
        'auth/popup-blocked': 'FALLBACK',
        'auth/popup-closed-by-user': 'FALLBACK',
      };
      return map[code] || 'GENERIC';
    },
    validateNickname(nick) {
      if (!nick) return 'EMPTY';
      if (nick.length < 3) return 'SHORT';
      if (nick.length > 18) return 'LONG';
      if (!/^[A-Za-zА-Яа-яЁё0-9_\-\.]+$/.test(nick)) return 'INVALID_CHARS';
      return 'OK';
    },
    passwordStrength(pw) {
      if (!pw) return 'NONE';
    ...
    passwordsMatch(a, b) { return !!a && a === b; },
    isValidEmail(em) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em || ''); },
    dashHas(key) {
